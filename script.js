...
function revealDestination(tripId) {
    // If tripId is not provided, get it from the data attribute
    if (!tripId) {
    ...
    setTimeout(() => {
        ...
        if ('Notification' in window && Notification.permission === 'granted') {
            showNotification('Destination Revealed!', `Your mystery destination is ${getDestinationById(trip.destinationId).name}!`);
        }
        
        showToast('success', 'Destination Revealed!', 'Your mystery destination has been revealed.');
    }, 1500);
}
